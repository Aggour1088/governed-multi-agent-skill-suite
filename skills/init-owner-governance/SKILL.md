...
6. Run 'scripts/sync_governance_adapters.py <project-root>' without '--apply' to preview thin adapter targets.
7. Apply only the reviewed targets. Keep adapters short and point them to the canonical constitution; do not duplicate policy text.
8. Copy only the artifact templates needed for a real feature into '.governance/features/<feature-id>/' when that feature begins.
9. Run 'scripts/check_governance_setup.py <project-root>' after any change.
10. Explain in plain language what becomes mandatory and what still requires owner approval.
