#!/usr/bin/env python3
"""Fail closed when a governed work-package record lacks verifiable file evidence."""
...
def validate_integrity(
    ...
    *,
    ...
    """Lint a record against files currently contained in project_root.

    This proves file integrity and declared-role consistency only. It does not
    certify that a command ran or authenticate a platform attestation.
    """
    errors: list[str] = []
    try:
    ...
    if not isinstance(implementation, str) or not FINGERPRINT.fullmatch(implementation):
        errors.append("implementation_fingerprint must be a sha256 fingerprint")
    else:
        try:
    ...
    kinds = {item.get("kind") for item in checks if isinstance(item, dict)}
    ...
    for item in checks:
        ...
        timestamp_error = validate_recorded_at(item.get("recorded_at"), now, float(max_evidence_age_hours))
        if timestamp_error:
            errors.append(f"{label} {timestamp_error}")
        errors.extend(validate_evidence_artifacts(root, item, label))
...
def validate(
    ...
    *,
    ...
    Use `validate_integrity()` only to lint contained files and record fields.
    A raw skill package has no trusted command runner or host identity provider,
    so a completed record must remain blocked for technical acceptance.
    """
